...
echo "SELECT *,concat(firstname,' ', lastname ) as name FROM individual_list where id in (SELECT individual_id from vaccine_history_list where individual_id = '{$id}')";
$user_qry = $conn->query("SELECT *,concat(firstname,' ', lastname ) as name FROM individual_list where id in (SELECT individual_id from vaccine_history_list where individual_id = '{$id}')");
...
<?php 
    ...
    $Get_name = "SELECT concat(firstname,' ', lastname ) as name FROM individual_list where id in (SELECT individual_id from vaccine_history_list where individual_id = '{$id}')";
...
<div class="card card-outline card-primary">
    <div class="card-header d-flex">
        <h5 class="card-title col-auto flex-grow-1">Manage Vaccine Details and Vaccination History</h5>
        <div class="col-auto">
            ...
            <div class="row">
                <div class="col-6">
                    <dl>
                        ...
                        <dt class="text-muted">User Name:</dt>
                        ...
                        <dd class="pl-4">
                            <?php if($status == 999): ?>
                                <span class="badge badge-success rounded-pill">Full Vaccinated</span>
                            <?php elseif($status >= 1): ?>
                                <span class="badge badge-primary rounded-pill">Vaccinated</span>
                            <?php else: ?>
    ...
    $(function(){
        ...
        $('#print').click(function(){
            ...
            _el.append('<div class="d-flex justify-content-center">'+
                      ...
                      '<h4 class="text-center">Manage Vaccine Details and Vaccination History</h4>'+
                      '</div>'+
                      '<div class="col-1 text-right">'+
                      '</div>'+
                      '</div><hr/>')
